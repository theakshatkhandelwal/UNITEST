# 🚀 Update Your Working Repository with Missing Features

This guide will help you add the missing features to your working repository: https://github.com/theakshatkhandelwal/UNITEST-EXAM.git
...
First, run this migration script to add the required columns:

```python
# Create a file: migrate_features.py
...
with app.app_context():
    ...
    try:
        ...
        print("✅ Added columns to quiz_question")
    except Exception as e:
        print(f"quiz_question columns may already exist: {e}")
    ...
    try:
        ...
        print("✅ Added columns to quiz_answer")
    except Exception as e:
        print(f"quiz_answer columns may already exist: {e}")
    ...
    try:
        ...
        print("✅ Added columns to quiz_submission")
    except Exception as e:
        print(f"quiz_submission columns may already exist: {e}")
    
    conn.commit()
    print("✅ Migration completed!")
```

Run: `python migrate_features.py`

---

## 📝 Step 2: Update Database Models

Add these to your `app.py` in the model definitions:
...
def take_shared_quiz(code):
    ...
    # Make sure to parse coding question data:
    if q.qtype == 'coding':
        try:
...
def submit_shared_quiz(code):
    ...
    # ... existing code for getting submission ...
    
    for q in questions:
        # ... existing MCQ and subjective handling ...
        ...
        # Save answer with coding fields
        ans = QuizAnswer(
            ...
            test_results_json=test_results_json,  # ✅ NEW
            passed_test_cases=passed_test_cases,  # ✅ NEW
            total_test_cases=total_test_cases  # ✅ NEW
        )
        db.session.add(ans)
    ...
    # ... rest of submission update ...
    db.session.commit()
```

### 4.3 Add `view_quiz_result` route:
...
### 4.4 Add `allow_student_retake` route:
...
Add countdown display in the "Your Shared Quiz History" section:
...
### 5.3 Create/Update `shared_quiz_results.html`:

Create this template to display results with coding question support.

---

## 📦 Step 6: Add API Endpoints for Code Testing

Add these routes for real-time code testing:
...
## ✅ Step 7: Update Quiz Generation

Update your `generate_quiz` function to support coding questions:
...
def generate_quiz(topic, difficulty_level, question_type="mcq", num_questions=5):
    ...
    if question_type == "coding":
        ...
        prompt = f"""
        Generate {num_questions} coding problems for the topic: {topic}
        Difficulty: {difficulty_level}
        
        For each problem, provide:
        1. Problem statement
        2. Sample input and output
        3. At least 3 test cases (some hidden)
        4. Starter code for Python, Java, C++, and C
        
        Format as JSON array.
        """
        # ... process response and create QuizQuestion objects with coding fields ...
```
...
2. Update your `app.py` with all the code above
